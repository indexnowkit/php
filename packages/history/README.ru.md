...
проверенный блок, хранилища поверх PDO или PSR-16, строку `check`, тела `history` и `status` (`historyRunner()`, `statusRunner()`; сами команды — `History\Console\HistoryCommand` и `StatusCommand`, классы, которые адаптер на symfony/console регистрирует над этими раннерами), описание хранилища — статические функции над частями, с двойниками `*For()` поверх
...
`pgsql`); `indexnowkit/console ^0.5` для команд (есть у каждого адаптера).
